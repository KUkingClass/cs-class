...
- 시스템 내에 여러 자원이 있는데, 1개의 자원을 할당할 때보다 많은 자원을 할당할 수록 데드락이 발생할 확률이 커지므로 자원의 총수, 현재 할당된 자원의 수를 기준으로 상태를 파악해서 안정 상태를 유지할 수 있는 범위 내에서 자원을 할당하는 것입니다.
  ...
  >
  > - 각 프로세스의 기대 자원과 비교해서 가용 자원이 크거나 같으면 할당한다.
  > - 가용 자원이 어떤 기대 자원보다 크지 않으면 할당하지 않는다.
  >
  > ![image-20220928185041419](./assets/image-20220928185041419.png)
  >
  > ![image-20220928185119676](./assets/image-20220928185119676.png)
...
- 데드락이 일어난 프로세스를 강제종료합니다. 또한 강제 종료된 프로세스가 실행하기 전에 시스템 역시 복구해야 합니다. 복구를 할 때는 명령어가 실행될 때 체크 포인트를 만들어서 가장 최근에 검사 시점으로 돌아가는 식으로 합니다.

- 이 때 강제종료하는 방법에도 여러가지가 있습니다.

  1. 전체를 동시에 강제 종료하기

     하지만 종료된 프로세스들이 다시 동시에 작업을 시작하면 또 데드락이 바생할 수가 있습니다. 그러므로 전체 종료된 후에 순차적으로 실행시켜야 하고, 이 때 순서의 기준도 필요합니다.

  2. 하나씩 종료하기

     데드락을 일으킨 프로세스 중 하나를 골라서 순서대로 종료하면서, 또 데드락의 상태 파악을 상태합니다. 그렇다면 이 때도 순서대로 종료하기 위해 기준이 필요합니다.
