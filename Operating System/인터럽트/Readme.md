...
![Untitled](Interrupt%2083bbbeb5d8894684a1bf58b56a44082c/Untitled.png)
...
![Untitled](Interrupt%2083bbbeb5d8894684a1bf58b56a44082c/Untitled%201.png)
...
- Q. 운영체제에게 CPU가 넘어가는 경우는 언제인가요?
    - A. 인터럽트 발생시, system call 발생 시, exception 발생 시

- Q. 인터럽트의 처리 순서를 간략히 적어주세요.

- Q. 입출력 수행, 기억장치 할당 및 오퍼레이터와 대화 등을 하기 위해 발생하는 인터럽트는 무엇인가요?

- Q. 인터럽트를 처리하는 중에 또 다른 인터럽트가 들어온다면 어떻게 처리되는지 간략히 적어주세요.

- Q. 시스템 콜과 인터럽트의 차이를 간략히 적어주세요.
    - A. 시스템콜은 시스템에 내장 된 서브 루틴에 대한 호출, 인터럽트는 CPU 외부의 이벤트로 인한 프로그램 제어 하는 것
    - 시스템 콜은 동기적 - 고정 된 시간에 발생, 프로세서는 복귀 할 위치 만 기억하면 됨
    - 인터럽트는 비동기적 - 예기치 않은 이벤트로 인해 언제든지 발생 가능, 복귀 할 위치와 시스템 상태를 모두 기억

- Q. 인터럽트와 트랩의 차이를 간략히 적어주세요.
    - A. 인터럽트는 하드웨어적인 흐름의 변화이고, 트랩은 소프트웨어적이라 할 수 있다.
